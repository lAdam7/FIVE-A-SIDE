...
class Goal extends Component {
    render() {
		 ...
		 * @var teamAGoals array Goals scored by teamA
		 * @var teamBGoals array Goals scored by teamB
		 */
        ...
        var kitNumber = (this.props.teamADetails === null || this.props.teamBDetails === null || this.props.teamAMembers === null || this.props.teamBMembers === null || this.props.teamAMembers === undefined || this.props.teamBMembers === undefined)
			? null
			: (event.teamID === this.props.teamADetails.id)
		...
		var teamAGoals = this.props.allEvents.filter(goal => goal.event === "Goal" && goal.teamA === true && goal.timestamp <= event.timestamp )
		var teamBGoals = this.props.allEvents.filter(goal => goal.event === "Goal" && goal.teamA === false && goal.timestamp <= event.timestamp )
        ...
        return (
            <View style={{ width: "80%", marginLeft: "10%", marginBottom: 10, backgroundColor: "#086134", borderRadius: 12 }}>
				<View style={{ flexDirection: "row", borderWidth: .75, borderTopLeftRadius: 12, borderTopRightRadius: 12, borderColor: "white" }}>
					<Image style={{ width: 25, height: 25, marginLeft: 5 }} source={require("../../../../assets/images/icons/events/football.png")} />
					<Text style={{ flex: 1, paddingLeft: 15, color: "white", fontWeight: "bold", textAlignVertical: "center", fontSize: 16 }}>{this.props.card}</Text>
					<Text style={{ flex: 1, textAlign: "right", paddingRight: 15, color: "white", fontWeight: "bold", textAlignVertical: "center" }}>{event.min}'</Text>
				</View>

				<View style={{ borderWidth: .75, borderTopWidth: 0, padding: 10, borderColor: "white" }}>
				...
				<View style={{ borderBottomLeftRadius: 12, borderBottomRightRadius: 12, borderWidth: .75, borderTopWidth: 0, padding: 4, borderColor: "white" }}>
					<Text style={{ textAlign: "center", color: "white", fontWeight: "bold", fontSize: 16 }}>{event.teamAAbbreviation}     {teamAGoals.length} - {teamBGoals.length}     {event.teamBAbbreviation}</Text>
				</View>
			</View>
        )
    }
